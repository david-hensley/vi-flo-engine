...
wds <- function(name){
  setwd(Sys.getenv("VI_FLO_DATA_ROOT"))
  datamap <- read.csv("datamap_engine.csv")
  return(datamap$absolute_path[datamap$named_path == name][1])
}

#' Takes a name for a specific path (i.e. a wd) and the local path.
#' Writes this into default_datamap.csv in the repo data file.
#' Use this to add a new named path or overwrite an existing one in the master list
...
set_named_path <- function(name, path){
  ...
  datamap <- read.csv("default_datamap.csv", stringsAsFactors = FALSE)
  # Check if path starts with data root prefix
  prefix <- Sys.getenv("VI_FLO_DATA_ROOT")
  if (!grepl(paste0("^", prefix), path)) {
    stop("Path must be within VI_FLO_DATA_ROOT (", prefix, ")\n",
         "Provided path: ", path, call. = FALSE)
  }
  # Strip prefix to get relative path
  prefix_with_slash <- paste0(prefix, "/")
  relative_path <- sub(paste0("^", prefix_with_slash), "", path)
  
  if (name %in% datamap$named_path){
    # Record the existing path
    existing_path <- datamap$path[datamap$named_path == name][1]
    existing_path_full <- paste0(Sys.getenv("VI_FLO_DATA_ROOT"), "/", existing_path)
    ...
    if (toupper(trimws(response)) == "Y") {
      message("Overwriting...")
      datamap$path[datamap$named_path == name] <- relative_path
    ...
    datamap <- rbind(datamap, new_row)
  }
  
  # Write updated datamap back to CSV
  write.csv(datamap, "default_datamap.csv", row.names = FALSE)
  message(paste0("✓ Updated default_datamap.csv with named path: ", name))
  invisible(TRUE)
}
